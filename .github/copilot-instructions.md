...
| Stack management script | PowerShell 5.1+ (`stack.ps1` — full stack, `sb.ps1` — Service Bus only) |
...
| Service Bus Proxy | `local-servicebus-proxy` | Custom (.NET 10) | 5672, 5300, 443 | Host-side HTTP GET `/health` via stack.ps1 or sb.ps1 |
...
- All services are health-checked; `servicebus-proxy` is probed from the host via stack.ps1 / sb.ps1 because it manages its own emulator containers.
...
To run **only the Service Bus proxy** (without databases):

```powershell
.\sb.ps1 start                # start Service Bus proxy only
.\sb.ps1 status               # verify proxy health
.\sb.ps1 stop                 # stop proxy without affecting databases
```
...
1. **Health checks are critical** — all services are health-checked; `servicebus-proxy` is probed from the host via stack.ps1 / sb.ps1 (no in-container healthcheck) so the management scripts and `depends_on` conditions work correctly.
